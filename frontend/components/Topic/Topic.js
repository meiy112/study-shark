...
export default function Topic({ route, navigation }) {
    ...
    async function fetchData() {
      try {
        const response = await fetch(
          "http://localhost:3000/topic/" + route.params.id + "/general-info"
        );
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const response = await fetch(
          "http://localhost:3000/topic/" + route.params.id + "/tags"
        );
      ...
      try {
        const response = await fetch(
          "http://localhost:3000/topic/" +
            route.params.id +
            "/studymaterial/?type=None&sort=dateCreated"
        );
    ...
    <View>
      <Appbar.Header style={{ backgroundColor: color.primary }}>
        ...
        <Appbar.Content
          title={topic.title}
          color="#FFFFFF"
          titleStyle={{ fontWeight: "600", fontSize: 20, fontFamily: "mon-sb" }}
        />
        <Appbar.Action
          icon="cog-outline"
          color="#FFFFFF"
...
function Info({ topic, tags }) {
  ...
  return (
    ...
    >
      ...
      >
        <Text
          style={{ color: "white", fontFamily: "mon-sb" }}
          variant="titleSmall"
        >
          Description
        </Text>
...
function StudyMaterialCard({
  ...
  const studyMaterialTitle =
    studyMaterial.title.length >= 17
      ? studyMaterial.title.substring(0, 14) + "..."
      : studyMaterial.title;
    ...
    >
      <TouchableOpacity onPress={onPress} disabled={isEditing}>
        ...
        <View style={{ marginTop: 15, marginBottom: 5 }}>
          <Text style={{ fontSize: 14, fontFamily: "mon-m" }}>
            {studyMaterialTitle}
          </Text>
          <View style={{ marginTop: 6 }}>
            <Text
              style={{ fontSize: 10, color: "#414141", fontFamily: "mon-l" }}
            >
              {studyMaterial.lastOpened}
            </Text>
          </View>
        </View>
        <CardFooter
          studyNoteType={studyMaterial.type}
          numItems={studyMaterial.numComponents}
        />
      </TouchableOpacity>
    </View>
  );
  ...
  return (
    ...
    >
      <View style={{ marginTop: 13, marginBottom: 13 }}>
        ...
        >
          <Text style={{ color: "#FFFFFF", fontSize: 10, fontFamily: "mon-m" }}>
            {studyMaterial.type.toUpperCase()}
          </Text>
        </View>
      </View>
      {isEditing && (
      ...
      <Portal>
        <Dialog visible={isVisible} onDismiss={hideDialog}>
          <Dialog.Content>
            <Text style={{ fontFamily: "mon-m" }} variant="bodyMedium">
              Are you sure you want to delete "{studyMaterial.title}" ?
            </Text>
          </Dialog.Content>
          <Dialog.Actions>
...
function CardFooter({ studyNoteType, numItems }) {
  ...
  return (
    <View style={{ flexDirection: "row", marginTop: 5, alignItems: "center" }}>
      ...
      <Text style={{ fontFamily: "mon-m", fontSize: 13 }}>
        {footerText[studyNoteType]}
      </Text>
    </View>
  );
}
